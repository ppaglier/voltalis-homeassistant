...
class VoltalisDevicePresetSelect(VoltalisDeviceEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        if option == VoltalisDevicePresetSelectOptionsEnum.OFF:
            await self.__set_manual_mode(is_on=False, mode=DeviceModeEnum.NORMAL)
            return
        ...
        # Handle ECOV mode
        if option == DeviceModeEnum.ECO and self.__has_ecov_mode:
            option = DeviceModeEnum.ECOV

        # Set the mode
        await self.__set_manual_mode(is_on=True, mode=cast(DeviceModeEnum, option))
